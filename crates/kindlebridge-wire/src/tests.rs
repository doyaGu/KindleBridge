...
fn in_flight_data_after_stream_close_consumes_connection_credit() {
    ...
    device
        .process_outbound(&header(Command::Close, 1, 1), FrameContext::default())
        .unwrap();
    let connection_credit_before = device.snapshot().connection_receive_credit;

    device
        .process_inbound(&data(1, 1, 10), FrameContext::default())
        .unwrap();
    assert_eq!(device.stream(1).unwrap().phase, StreamPhase::Closed);
    assert_eq!(
        device.snapshot().connection_receive_credit,
        connection_credit_before - 10
    );
}
